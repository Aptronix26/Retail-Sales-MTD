...
| Available-period revenue YoY % | (Current full-month revenue through 31 Aug 2026 − source-supplied prior-year revenue through 23 Aug 2025) ÷ source-supplied prior-year revenue × 100 |
...
For the 31 August 2026 snapshot, `Aptronix_PLF_SKC` and `Ecom-TL` are outside the approved 69-store scope. The trade-in source has no impossible quantity outliers requiring quarantine; 420 loan quantities assigned to an unmapped `#N/A` branch remain excluded. The supplied prior-year comparison is through 23 August 2025, so the dashboard explicitly labels it available-period rather than same-date YoY. Store-level growth displays `Low LY base` when comparable revenue is below ₹1 lakh.
